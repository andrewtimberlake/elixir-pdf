defmodule Pdf.ExternalFont do
  ...
  Examples:
    ...
    123
  """
  def width(font, <<char_code::utf8>> = str) when is_binary(str) do
  ...
  def kern_text(font, [first, second | tail]) do
    font.kern_pairs
    |> Enum.find(fn {f, s, _amount} -> f == first && s == second end)
    |> case do
      {f, _s, amount} ->
        [f, amount | kern_text(font, [second | tail])]

      nil ->
        [<<head::binary>> | tail] = kern_text(font, [second | tail])
        [<<first, head::binary>> | tail]
    end
  end

  def kern_text(_font, [char]), do: [<<char>>]
